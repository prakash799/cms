    ...
    <c:forEach items="${userlist}" var="element">
      <tr>
        ...
        <form action="updateUrl" method="post">
        	<input type="hidden" id="id" name="id" value="${element.id}">
        	<input type="button" class="btn btn-primary btn-xs" id="userupdate${element.id}" onclick="fillForm('${element.id}')" value="Update" >
        </form>
        </div>
        </div>
        </td>
	 ...
	 <div id = "dialog-1" title = "Update User Details" style="display: none;border: solid sliver 1px; background-color: white;">
         <form:form name="registratoin" action="udpateuserdetails" class="form-horizontal" method="post" modelAttribute="user">
         <form:input type="hidden" id="userid" path="id" name="userId"/>
        	<div class="form-group">
				<div class="col-sm-4 col-sm-offset-4">
					<form:input type="text" name="name" id="name" path="name"
						class="text ui-widget-content ui-corner-all" placeholder="Enter your full name"
			...
			<div class="col-sm-4 col-sm-offset-4">
				...
				<input type="reset" value="Reset" class="btn btn-success" onSubmit="return test();">
			</div>
         </form:form>
         </div>
</body>
